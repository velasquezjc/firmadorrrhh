...
/*******************/
		String [] operaciones2 =  new String [options.getDatas().length];//siempre va a ser una operacion cuando se selecciona un archivo
		operaciones2[0] = options.getOperation().name();//params.get(ProtocolConstants.OPERATION_PARAM);
		options.setOperaciones(operaciones2);
		...
		//generamos dialogos para guardar cada firma generada (solo operaciones de firma)
		for (int i = 0; i < nFirmas; i++)
		{
			switch(UrlParametersToSign.Operation.getOperation(operaciones[i])) {
			case SIGN:
