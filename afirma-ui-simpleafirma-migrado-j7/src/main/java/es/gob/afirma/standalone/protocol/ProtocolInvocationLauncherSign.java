...
final class ProtocolInvocationLauncherSign {
	...
	static String processSign(final UrlParametersToSign options, final boolean bySocket) throws SocketOperationException {
		...
		//TODO: hacer que desde el navegador se envie un array de datos (por mas que sea solo un dato)para asi eliminar
		//data y dejarlo a datas
		if (options.getData() == null) {
			...
			try {
				...
				//options.setData(data); //se ubica el archivo seleccionado como un item de datos
				byte[][] datos = new byte[1][];
				datos[0]=data;
				options.setDatas(datos);
				String [] operaciones =  new String [datos.length];//siempre va a ser una operacion cuando se selecciona un archivo
				operaciones[0] = options.getOperation().name();//params.get(ProtocolConstants.OPERATION_PARAM);
				options.setOperaciones(operaciones);
			}
		...
		if (signer == null) {
			if (Operation.SIGN == options.getOperation()) {
				if (DataAnalizerUtil.isPDF(options.getDatas()[0])) {
				...
				else if (DataAnalizerUtil.isFacturae(options.getDatas()[0])) {
				...
				else if (DataAnalizerUtil.isXML(options.getDatas()[0])) {
				...
				try {
					signer = AOSignerFactory.getSigner(options.getDatas()[0]);
				}
