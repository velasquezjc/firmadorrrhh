...
final class ProtocolInvocationLauncherSignxxx {
	...
	private ProtocolInvocationLauncherSignxxx() {
		// No instanciable
	}
	...
	static String processSign(final UrlParametersToSign options, final boolean bySocket) throws SocketOperationException {
		...
		int nFirmas = options.getData().length;
		final byte[][] sign = new byte [nFirmas][];
		//String [] operaciones = options.getOperation();
