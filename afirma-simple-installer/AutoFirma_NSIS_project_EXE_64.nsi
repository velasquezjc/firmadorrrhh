...
;la instalacion
  ...
  !define MUI_HEADERIMAGE_BITMAP "logomds.bmp"
  !define MUI_HEADERIMAGE_UNBITMAP "logomds.bmp"
  !define MUI_WELCOMEFINISHPAGE_BITMAP "logoportadamds.bmp"
  !define MUI_UNWELCOMEFINISHPAGE_BITMAP "logoportadamds.bmp"
...
VIAddVersionKey "ProductName" "AutoFirmaMDS"
VIAddVersionKey "ProductVersion" "${VERSION}"
VIAddVersionKey "FileVersion" "${VERSION}"
VIAddVersionKey "LegalCopyright" "GPL"
VIAddVersionKey "FileDescription" "AutoFirmaMDS (64 bits)"
...
Function createConfigPage
  !insertmacro MUI_HEADER_TEXT "Opciones de integración avanzadas" "Seleccione las opciones de integración que desee que configure AutoFirmaMDS"
...
OutFile AutoFirma64/AutoFirmaMDS_64_v1_8_0_installer.exe
...
Name "AutoFirmaMDS"
Caption "Instalador de AutoFirmaMDS (Cliente PC)"
Icon logomds2-modified.ico
...
InstallDir "$PROGRAMFILES64\AutoFirmaMDS"
...
UninstallText "Desinstalador de AutoFirmaMDS."
...
Section "Programa" sPrograma
	...
	pop $3
	${If} $1 != 1
		MessageBox MB_OK "No se puede instalar AutoFirmaMDS 64 bits en un entorno 32 bits." 
		Quit
	${EndIf}
	...
	${If} $R1 != ""
		...
		${If} $R2 = 0
		  MessageBox MB_OK "Esta versión de AutoFirmaMDS ya está instalada." 
		  Quit
		${ElseIf} $R2 <> 2
		  MessageBox MB_OK "La versión actual de AutoFirmaMDS es más nueva que la que se quiere instalar."
		  Quit
		${EndIf}
...
;	File /r java64\jre
	...
	File  AutoFirma64\AutoFirmaMDS.exe
	File  AutoFirma64\AutoFirmaConfiguradorMDS.exe
;	File  AutoFirma64\AutoFirmaCommandLine.exe
	File  licencia.txt
	File  logomds2-modified.ico
	...
	${If} $Shorcut_Integration_Checkbox_State == 1
		CreateShortCut "$DESKTOP\AutoFirmaMDS.lnk" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	${Endif}

	;Si se ha configurado, creamos el grupo de accesos en el menu inicio
	${If} $StartMenu_Integration_Checkbox_State == 1
		CreateDirectory "$SMPROGRAMS\AutoFirmaMDS"
		CreateShortCut "$SMPROGRAMS\AutoFirmaMDS\AutoFirmaMDS.lnk" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
		CreateShortCut "$SMPROGRAMS\AutoFirmaMDS\Desinstalar.lnk" "$INSTDIR\uninstall.exe"
	${Endif}

	;Anade una entrada en la lista de "Program and Features"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$PATH" "DisplayName" "AutoFirmaMDS"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$PATH" "UninstallString" "$INSTDIR\uninstall.exe"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$PATH" "DisplayIcon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	...
	;Registro
	;CascadeAfirma.reg
	/*WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.sign" "" "Firmar con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.sign" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.sign\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe sign -gui -i "%1"'

	;Generar huella archivos
 	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.hashFile" "" "Generar huella digital con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.hashFile" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.hashFile\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe createdigest -i "%1"'
	...
	WriteRegStr HKEY_CLASSES_ROOT "Directory\shell\afirma.hashDirectory" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT "Directory\shell\afirma.hashDirectory\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe createdigest -i "%1"'

	;Comprobar huella .hash
 	WriteRegStr HKEY_CLASSES_ROOT ".hash\shell\afirma.hash" "" "Comprobar huella digital con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT ".hash\shell\afirma.hash" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT ".hash\shell\afirma.hash\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe checkdigest -i "%1"'

	;Comprobar huella .hashb64
 	WriteRegStr HKEY_CLASSES_ROOT ".hashb64\shell\afirma.hasbh64" "" "Comprobar huella digital con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT ".hashb64\shell\afirma.hasbh64" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT ".hashb64\shell\afirma.hasbh64\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe checkdigest -i "%1"'
	
	;Comprobar huella .hashfiles
 	WriteRegStr HKEY_CLASSES_ROOT ".hashfiles\shell\afirma.hashfiles" "" "Comprobar huella digital con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT ".hashfiles\shell\afirma.hashfiles" "Icon" "$INSTDIR\$PATH\AutoFirmaMDS.exe"
	WriteRegStr HKEY_CLASSES_ROOT ".hashfiles\shell\afirma.hashfiles\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe checkdigest -i "%1"'
*/
	;Verify
	; .csig
	...
	WriteRegStr HKEY_CLASSES_ROOT ".csig\shell\Verify" "" "Verificar con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT ".csig\shell\Verify\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe verify -gui -i "%1"'
	...
	WriteRegStr HKEY_CLASSES_ROOT ".xsig\shell\Verify" "" "Verificar con AutoFirmaMDS"
	WriteRegStr HKEY_CLASSES_ROOT ".xsig\shell\Verify\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe verify -gui -i "%1"'
	
	;Protocolo afirma
	WriteRegStr HKEY_CLASSES_ROOT "afirma" "" "URL:Afirma Protocol"
	WriteRegStr HKEY_CLASSES_ROOT "afirma\DefaultIcon" "" "$INSTDIR\$PATH\logomds2-modified.ico"
	WriteRegStr HKEY_CLASSES_ROOT "afirma" "URL Protocol" ""
	WriteRegStr HKEY_CLASSES_ROOT "afirma\shell\open\command" "" '$INSTDIR\$PATH\AutoFirmaMDS.exe "%1"'

	; Eliminamos los certificados generados en caso de que existan por una instalacion previa
	IfFileExists "$INSTDIR\$PATH\FirmaDigital_ROOT_navegador.cer" 0 +1
	Delete "$INSTDIR\$PATH\FirmaDigital_ROOT_navegador.cer"
	IfFileExists "$INSTDIR\$PATH\firmadigitalkeystore.pfx" 0 +1
	Delete "$INSTDIR\$PATH\firmadigitalkeystore.pfx"
	...
	${Endif}
	ExecWait '"$INSTDIR\$PATH\AutoFirmaConfiguradorMDS.exe" $R0 /passive'
	...
	Sleep 2000
	Push "$INSTDIR\$PATH\FirmaDigital_ROOT_navegador.cer"
...
Function un.DeleteCertificate
  ...
  ${Else}
	...
	StrCpy $7 "Firma Digital ROOT Nav"
	StrCpy $8 "Firma Digital ROOT Nav"
  ${EndIf}
...
;		MessageBox MB_OK|MB_DEFBUTTON1|MB_ICONEXCLAMATION 'Cierre el navegador Google Chrome para continuar con la desinstalación de AutoFirma.' IDOK loopChrome
	...
	ExecWait '"$INSTDIR\$PATH\AutoFirmaConfiguradorMDS.exe" -uninstall /passive'
	...
	Delete "$DESKTOP\AutoFirmaMDS.lnk"
...
Function RemoveOldVersions
	...
	CheckAutoFirmaVersion:
		...
		StrCmp $3 "AutoFirmaMDS" 0 searchAutoFirmaLoop
		...
		StrCmp $3 "AutoFirmaMDS" 0 End
		; Informamos de que existe una version anterior, ofrecemos el eliminarla y cerramos el
		; instalador si no se quiere desinstalar
		MessageBox MB_YESNO "Existe una versión anterior de AutoFirmaMDS en el equipo. ¿Desea desinstalarla?" /SD IDYES IDNO Exit
			Goto UninstallOlderVersion
...
Function AddToPath
  ...
  IntCmp $4 234 0 +3 +3 ; $4 == ERROR_MORE_DATA
    DetailPrint "El PATH es demasiado largo. No se le agregará la ruta de AutoAfirmaMDS."
    Goto done
  ...
  IntCmp $2 ${NSIS_MAX_STRLEN} +3 +3 0
    DetailPrint "La ruta de AutoFirmaMDS hace que el PATH sea demasiado largo. No se agregará"
    Goto done
  ; Append dir to PATH
