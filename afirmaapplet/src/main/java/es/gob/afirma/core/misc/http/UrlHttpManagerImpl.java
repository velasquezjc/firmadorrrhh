...
public class UrlHttpManagerImpl implements UrlHttpManager {
	...
	private byte[] readUrl(final String url,
		...
		if (!url.contains("?") && Method.POST.equals(method)) { //$NON-NLS-1$
			...
			return readUrl(url, timeout, contentType, Method.GET, cookieManager);
		}
	 ...
	 * En HTTPS no se hacen comprobaciones del certificado servidor.
	 * @param url URL a leer.
	 * @param cookieManager Gestor de <i>cookies</i> para la conexi&oacute;n HTTP.
	 ...
	 * @throws IOException Si no se puede leer la URL */
	@Override
	public byte[] readUrlByPost(final String url,
			                    final int timeout,
			                    final String contentType,
			                    final CookieManager cookieManager) throws IOException {
		return readUrl(url, timeout, contentType, Method.POST, cookieManager);
	}
	 ...
	 * @throws IOException Si no se puede leer la URL. */
	@Override
	public byte[] readUrlByGet(final String url,
			                   final CookieManager cookieManager) throws IOException {
		return readUrl(url, DEFAULT_TIMEOUT, null, Method.GET, cookieManager);
	}
	...
	public byte[] readUrlByPost(final String url, final int timeout, final String contentType) throws IOException {
		return readUrl(url, timeout, contentType, Method.POST, null);
	}
	...
	public byte[] readUrlByGet(final String url) throws IOException {
		return readUrl(url, DEFAULT_TIMEOUT, null, Method.GET, null);
	}
