...
public class AutoFirmaConfigurator implements ConsoleListener {
	...
	private static final String KS_FILENAME = "firmadigitalkeystore.pfx";
	...
	public void configureAutoFirma() throws GeneralSecurityException, ConfigurationException, IOException {
		...
		//this.mainScreen.showConsole(); //ver si no es necesario ,o usar la version silent del configurador
	...
	public static void main(final String[] args) {
		...
		try {
			configurator.configureAutoFirma(); //configurator.uninstall();			
			//System. out. println(ConfiguratorUtil.getApplicationDirectory().getAbsolutePath());								
			
		}
		catch (final KeyStoreException e) {
			LOGGER.severe("Capturado error no controlado: " + e); //$NON-NLS-1$
			//no doy mensaje porque puede que no se haya instalado el certificado en un perfil de firefox pero
			//que si para el usuario actual
			...
			LOGGER.severe("Error en la configuracion de AutoFirma: " + e); //$NON-NLS-1$
			ConsoleManager.showErrorMessage(
				configurator.getParentComponent(),
//				Messages.getString("AutoFirmaConfigurator.0") //$NON-NLS-1$
				Messages.getString("Puede que su navegador no pueda comunicarse con la app Firma Digital MDS. Importe el certificado Temp/FirmaDigital_ROOT.cer en su navegador.") //$NON-NLS-1$
			);
			/*si hubo algun error intento borrar los certificados y el keystore de la app, no es necesario borrar
			 * el .cer de la carpeta temporal porque siempre en una nueva instalacion se pisa*/
//			configurator.uninstall();				
//			final File appDir = new File(ConfiguratorUtil.getApplicationDirectory(), KS_FILENAME);
//			if (appDir.exists()) {				
				//ConfiguratorUtil.deleteDir(new File(appDir, KS_FILENAME));
//				appDir.delete();
//			}	
