...
public enum ValidationResult {
	...
	/**No se pudo instanciar el verificador porque no se configuro*/
	VERIFICADOR_NO_SOPORTADO(12);
	...
	private static final int CODE_VERIFICADOR_NO_SOPORTADO = 12;

	private final int resultCode;
	private ValidationResult(final int code) {
		if (code < CODE_VALID || code > CODE_VERIFICADOR_NO_SOPORTADO) {
			throw new IllegalArgumentException(
				"El codigo de resultado debe estar comprendido entre 0 y 12: " + code //$NON-NLS-1$
			);
		}
	...
	public String toString() {
		switch(this.resultCode) {
			...
			case CODE_VERIFICADOR_NO_SOPORTADO:
				return CertValidationMessages.getString("ValidationResult.12"); //$NON-NLS-1$
			default:
				throw new IllegalStateException(
					"El codigo de resultado debe estar comprendido entre 0 y 12: " + this.resultCode //$NON-NLS-1$
				);
		}
	}
	...
	public void check() throws CertificateException {
		switch(this.resultCode) {
			...
			case CODE_VERIFICADOR_NO_SOPORTADO:
				throw new CertificateException("No se ha podido instanciar el verificador del certificado"); //$NON-NLS-1$
			default:
				throw new IllegalStateException(
					"El codigo de resultado debe estar comprendido entre 0 y 12: " + this.resultCode //$NON-NLS-1$
				);
