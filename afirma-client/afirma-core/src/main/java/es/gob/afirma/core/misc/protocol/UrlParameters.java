...
public abstract class UrlParameters {
	...
	private String defaultKeyStoreLib = null;//ver
    ...
    //TODO: procedimiento modificado para soportar envio de libreria por parametro en caso de repositorio PKCS11
	protected static String verifyDefaultKeyStoreName(final Map<String, String> params) {
		...
		if (params.containsKey(KEYSTORE)) {
			//obtengo solo el nombre del almacen
			ksValue = params.get(KEYSTORE);
			separatorPos = ksValue.indexOf(':');
			if(separatorPos==-1){//entonces solo contiene el nombre del almacen
	...
	/*no estoy seguro si aun va o ya se deja de usar por ya no dar compatibilidad con afirma 1.4*/
